...
// The ID of the route we just created in the backend.
const KNOWN_ROUTE_ID = "ASR-JUC-01";

export default function UserTestScreen() {
  const router = useRouter();

  const handleTrackPress = () => {
    console.log(`--- Hardcoded Test: Navigating to /track with route_id: ${KNOWN_ROUTE_ID} ---`);
    router.push({ pathname: '/track', params: { busId: KNOWN_ROUTE_ID } });
  };

  return (
    <View style={styles.container}>
      <Text style={styles.header}>User App Test</Text>
      <Text style={styles.info}>This button will track the hardcoded route: {KNOWN_ROUTE_ID}</Text>
      <Button
        title={`Track Route ${KNOWN_ROUTE_ID}`}
